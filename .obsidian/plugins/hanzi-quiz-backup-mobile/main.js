...
'마지막 업데이트: ' + updateTime + '\n';
    ...
    async loadAllQuestions() {
        ...
        const files = allFiles.filter(file => {
            ...
            const notQuestionList = !file.path.includes('문제목록');
            
            if (startsWith && notQuestionList) {
                console.log(`MATCHED: ${file.path}`);
            }
            
            return startsWith && notQuestionList;
        });
...
class HanziQuestionModal extends Modal {
    ...
    createImageUploader(container, getValue, setValue, label = '이미지') {
        ...
        // 버튼 컨테이너
        const buttonRow = imageContainer.createDiv({ cls: 'image-button-row' });
        ...
        // 클립보드 이미지 붙여넣기 버튼
        const clipboardBtn = buttonRow.createEl('button', {
            text: '📋 붙여넣기',
            ...
            try {
                ...
                for (const item of clipboardItems) {
                    ...
                    if (imageType) {
                        ...
                        // 상대 경로 형식으로 저장
                        setValue(`![[첨부파일/${fileName}]]`);
                        imageInput.value = `![[첨부파일/${fileName}]]`;
                        
                        new Notice(`✅ ${label} 클립보드 이미지 저장 완료`);
                        updateImagePreview();
                        imageFound = true;
                        break;
                    }
        ...
        const updateImagePreview = async () => {
            ...
            img.onerror = () => {
                ...
                previewContainer.createEl('div', {
                    ...
                    attr: { style: 'padding: 10px; text-align: center; color: var(--text-muted); font-size: 0.8em;' }
                });
            };
        };
    ...
    async onOpen() {
        ...
        `;
        header.createEl('h2', { text: this.existingQuestion ? '✏️ 문제 수정' : '📝 새 문제 만들기' });
        ...
        new Setting(form)
            .setName('핵심 키워드')
            .setDesc('문제의 핵심 키워드를 입력하세요 (한자, 단어, 개념 등)')
            .addText(text => text
                .setPlaceholder('예: 愛, 사랑, 經濟 등')
        ...
        folderDropdown.style.flex = '1';
        folderDropdown.style.padding = '8px';
        ...
        const imageSetting = new Setting(form)
            .setName('이미지 (선택)')
            .setDesc('이미지를 업로드하거나 URL을 입력하세요');

        const imageContainer = imageSetting.controlEl.createDiv({ cls: 'image-upload-container' });
        ...
        const imageInput = imageContainer.createEl('input', {
            type: 'text',
            placeholder: '이미지 URL 또는 [[파일명]] 형식',
            value: this.question.image || ''
        });
        imageInput.style.width = '100%';
        imageInput.style.padding = '8px';
        imageInput.addEventListener('input', (e) => {
            this.question.image = e.target.value;
            updateImagePreview();
        });
            ...
            try {
                ...
                for (const item of clipboardItems) {
                    ...
                    if (imageType) {
                        ...
                        // 상대 경로 형식으로 저장
                        this.question.image = `![[첨부파일/${fileName}]]`;
                        imageInput.value = this.question.image;
                        
                        new Notice(`✅ 클립보드 이미지 저장 완료`);
                        updateImagePreview();
                        imageFound = true;
                        break;
                    }
                ...
                console.error('클립보드 이미지 붙여넣기 실패:', error);
                new Notice('❌ 클립보드 이미지 읽기 실패: ' + error.message);
            }
        };
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                new Notice(`✅ 이미지 업로드 완료`);
                updateImagePreview();
            } catch (error) {
                console.error('이미지 업로드 실패:', error);
                new Notice('❌ 이미지 업로드에 실패했습니다.');
            }
        });
        ...
        const updateImagePreview = async () => {
            if (!this.question.image || !this.question.image.trim()) {
                previewContainer.style.display = 'none';
                return;
            }

            let imageUrl = this.question.image.trim();
        ...
        renderOptions = () => {
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                optionWrapper.style.border = '1px solid var(--background-modifier-border)';
                optionWrapper.style.borderRadius = '6px';
        ...
        // 힌트 클립보드 붙여넣기 버튼
        const hintPasteBtnRow = hintContainer.createDiv({ cls: 'paste-btn-row' });
        hintPasteBtnRow.style.display = 'flex';
        hintPasteBtnRow.style.gap = '8px';
        hintPasteBtnRow.style.marginBottom = '4px';
        
        const hintPasteBtn = hintPasteBtnRow.createEl('button', {
            text: '📋 클립보드 붙여넣기',
            cls: 'paste-btn'
        });
        hintPasteBtn.type = 'button';
        hintPasteBtn.style.padding = '6px 12px';
        hintPasteBtn.style.fontSize = '0.9em';
        hintPasteBtn.style.cursor = 'pointer';
        
        // 힌트 입력 영역
        const hintInput = hintContainer.createEl('textarea', {
            placeholder: '틀렸을 때 보여줄 힌트',
            value: this.question.hint || ''
        });
        hintInput.rows = 2;
        hintInput.style.width = '100%';
        hintInput.style.padding = '8px';
        hintInput.style.resize = 'vertical';
        ...
        // 힌트 클립보드 버튼 클릭
        hintPasteBtn.addEventListener('click', async () => {
            try {
                const text = await navigator.clipboard.readText();
                if (text) {
        ...
        noteContainer.style.gap = '8px';
        noteContainer.style.width = '100%';
        
        // 노트 클립보드 붙여넣기 버튼
        const notePasteBtnRow = noteContainer.createDiv({ cls: 'paste-btn-row' });
        notePasteBtnRow.style.display = 'flex';
        notePasteBtnRow.style.gap = '8px';
        notePasteBtnRow.style.marginBottom = '4px';
        
        const notePasteBtn = notePasteBtnRow.createEl('button', {
            text: '📋 클립보드 붙여넣기',
            cls: 'paste-btn'
        });
        notePasteBtn.type = 'button';
        notePasteBtn.style.padding = '6px 12px';
        notePasteBtn.style.fontSize = '0.9em';
        notePasteBtn.style.cursor = 'pointer';
        
        // 노트 입력 영역
        const noteInput = noteContainer.createEl('textarea', {
            placeholder: '추가 설명이나 기억할 내용',
        ...
        // 노트 클립보드 버튼 클릭
        notePasteBtn.addEventListener('click', async () => {
            try {
                const text = await navigator.clipboard.readText();
                if (text) {
                    noteInput.value = text;
                    this.question.note = text;
                    noteInput.focus();
                    new Notice('📋 클립보드 내용이 붙여넣어졌습니다.');
                }
            } catch (err) {
...
class FolderManagementModal extends Modal {
    ...
    async onOpen() {
        ...
        contentEl.createEl('h2', { text: '📂 폴더 관리 v3' });
    ...
    async copyFolder(sourceFolder, targetFolder) {
        try {
            ...
            for (const file of sourceFiles) {
                ...
                await this.app.vault.create(`${targetPath}/${file.name}`, updatedContent);
                copiedCount++;
            }
            ...
            // 문제목록 템플릿 생성
            await this.plugin.updateQuestionListTemplate(targetFolder);
            
            new Notice(`✅ ${targetFolder} 폴더 생성 완료 (${copiedCount}개 문제)`);
            this.onOpen();
        } catch (error) {
...
class QuizPlayModal extends Modal {
    ...
    showQuestion() {
        ...
        if (question.image && question.image.trim()) {
            ...
            let imageUrl = question.image.trim();
        ...
        if ((question.hint && question.hint.trim()) || (question.hintImage && question.hintImage.trim())) {
            ...
            if (question.hintImage && question.hintImage.trim()) {
                ...
                const img = hintImgContainer.createEl('img', {
                    attr: {
                        src: imageUrl,
                        style: 'width: 100%; max-width: 400px; height: auto; border-radius: 6px; cursor: zoom-in; transition: transform 0.2s;'
                    }
                });
                ...
                img.onerror = () => {
                    hintImgContainer.setText('⚠️ 힌트 이미지를 불러올 수 없습니다.');
                };
            }
        ...
        options.forEach((option, index) => {
            ...
            if (question.optionImages && question.optionImages[originalIndex] && question.optionImages[originalIndex].trim()) {
                ...
                optionImageContainer.style.maxHeight = '100px';
                optionImageContainer.style.overflow = 'hidden';
                optionImageContainer.style.display = 'flex';
                optionImageContainer.style.justifyContent = 'center';
                optionImageContainer.style.alignItems = 'center';
                optionImageContainer.style.cursor = 'zoom-in';
                
                let imageUrl = question.optionImages[originalIndex].trim();
                ...
                const img = optionImageContainer.createEl('img', {
                    attr: { 
                        src: imageUrl, 
                        alt: `선택지 ${index + 1} 이미지`
                    }
                ...
                noteBtn.addEventListener('click', () => {
                    ...
                    if (noteDisplay) {
                        ...
                        if (question.noteImage && question.noteImage.trim()) {
                            let imageUrl = question.noteImage.trim();
                            ...
                            const noteImg = noteDisplay.createEl('img', {
                                attr: {
                                    src: imageUrl,
                                    style: 'width: 100%; max-width: 350px; height: auto; margin-top: 10px; border-radius: 6px; display: block;'
                                }
