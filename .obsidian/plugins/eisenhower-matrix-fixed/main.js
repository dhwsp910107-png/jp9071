...
`;
    ...
    if (currentPeriod === 'weekly') {
        ...
        this.tasks.yearly[quadrant].push(newTask);
        
        // 연간 파일 동기화
        if (this.plugin.settings.autoSyncToNote) {
            await this.syncTaskToYearlyFile(quadrant, taskText);
        }
    } else {
    ...
    if (this.plugin.settings.autoSyncToNote && currentPeriod !== 'weekly' && currentPeriod !== 'yearly') {
...
async syncTaskToYearlyFile(quadrant, taskText) {
    try {
        const year = this.currentYear || new Date().getFullYear();
        const settings = this.plugin?.settings || {};
        const mainFolder = settings.mainFolder || 'Eisenhower Matrix';
        const yearlyFolder = settings.yearlyFolder || 'Yearly';
        const fileName = `${mainFolder}/${yearlyFolder}/${year}년-계획.md`;
        
        // 파일이 없으면 생성
        let file = await this.safeGetOrCreateFile(fileName, this.getDefaultYearlyContent());
        if (!file) {
            safeLog.warn('연간 파일을 생성할 수 없습니다:', fileName);
            return;
        }
        ...
        if (match) {
            ...
            safeLog.log('연간 파일에 할일 추가 완료:', taskText);
        } else {
            safeLog.warn('연간 파일에서 해당 섹션을 찾을 수 없습니다:', quadrantNames[quadrant]);
        }
    } catch (error) {
        safeLog.error('연간 파일 동기화 오류:', error);
    }
}
 ...
 async updateYearlyTaskStatus(quadrant, index, taskText, completed) {
    ...
    try {
        ...
        // 파일이 없으면 생성
        let file = await this.safeGetOrCreateFile(fileName, this.getYearlyTemplate());
        if (!file) return;
        ...
        if (completed && content.includes(uncheckedPattern)) {
            ...
            safeLog.log('연간 노트 완료 처리:', taskText);
        } else if (!completed && content.includes(checkedPattern)) {
            const uncheckedTask = `- [ ] ${taskText}`;
            const escapedText = taskText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const checkedWithTimePattern = new RegExp(`- \\[x\\] ${escapedText} \\*완료: \\d{2}:\\d{2}\\*`);
            ...
            content = this.sanitizeYAMLFrontmatter(content);
            await this.app.vault.modify(file, content);
            safeLog.log('연간 노트 미완료 처리:', taskText);
        }
    } catch (error) {
...
// ========================================
        ...
        quadrants.forEach(q => {
            ...
            tasks.forEach((task, taskIndex) => {
                ...
                removeBtn.addEventListener('click', async () => {
                    await this.removeWeeklyTask(q.id, taskIndex, week);
                });
            });
    ...
    async removeWeeklyTask(quadrant, index, week) {
        try {
            const weekKey = `week${week}`;
            if (this.tasks.weekly[weekKey]?.[quadrant]?.[index]) {
                const task = this.tasks.weekly[weekKey][quadrant][index];
                this.tasks.weekly[weekKey][quadrant].splice(index, 1);
                ...
                if (this.plugin.settings.deleteFromNote && this.plugin.settings.autoSyncToNote) {
                    await this.removeTaskFromNote(quadrant, typeof task === 'object' ? task.text : task, 'weekly', week);
                }
                
                const fileStatus = this.plugin.settings.deleteFromNote ? '' : ' (뷰에서만)';
                new Notice(`🗑️ ${week}주차 삭제됨${fileStatus}: ${typeof task === 'object' ? task.text : task}`);
            }
        } catch (error) {
            safeLog.error('주간 할일 삭제 오류:', error);
            new Notice('❌ 주간 할일 삭제 실패');
        }
    }
    ...
    showDayTasks(selectedDate) {
        ...
        quadrants.forEach(quadrant => {
            ...
            tasks.forEach((task, index) => {
                ...
                const removeBtn = taskItem.createEl('button', { cls: 'em-btn-remove', text: '✗' });
                removeBtn.onclick = async () => await this.removeMonthlyTask(quadrant.key, index, day);
            });
    ...
    async removeMonthlyTask(quadrant, index, day) {
        try {
            const dayKey = `day${day}`;
            if (this.tasks.monthly[dayKey]?.[quadrant]?.[index]) {
                const task = this.tasks.monthly[dayKey][quadrant][index];
                this.tasks.monthly[dayKey][quadrant].splice(index, 1);
                ...
                if (this.plugin.settings.deleteFromNote && this.plugin.settings.autoSyncToNote) {
                    await this.removeTaskFromNote(quadrant, typeof task === 'object' ? task.text : task, 'monthly', day);
                }
                
                const fileStatus = this.plugin.settings.deleteFromNote ? '' : ' (뷰에서만)';
                new Notice(`🗑️ ${day}일 삭제됨${fileStatus}: ${typeof task === 'object' ? task.text : task}`);
            }
        } catch (error) {
            safeLog.error('월간 할일 삭제 오류:', error);
            new Notice('❌ 월간 할일 삭제 실패');
        }
    }
...
*생성 시간: ${now.toLocaleString('ko-KR')}*`;
    ...
    renderYearlyQuadrantTasks(container, quadrant) {
        ...
        currentTasks.forEach((taskData, index) => {
            ...
            removeBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                await this.removeYearlyTask(quadrant, index);
            });
        });
    }

    async removeYearlyTask(quadrant, index) {
        try {
            if (this.tasks.yearly?.[quadrant]?.[index]) {
                const task = this.tasks.yearly[quadrant][index];
                this.tasks.yearly[quadrant].splice(index, 1);
                
                this.saveToStorage();
                this.refreshView();
                
                if (this.plugin.settings.deleteFromNote && this.plugin.settings.autoSyncToNote) {
                    await this.removeYearlyTaskFromNote(quadrant, typeof task === 'object' ? task.text : task);
                }
                
                const fileStatus = this.plugin.settings.deleteFromNote ? '' : ' (뷰에서만)';
                new Notice(`🗑️ 연간 목표 삭제됨${fileStatus}: ${typeof task === 'object' ? task.text : task}`);
            }
        } catch (error) {
            safeLog.error('연간 할일 삭제 오류:', error);
            new Notice('❌ 연간 할일 삭제 실패');
        }
    }

    async removeYearlyTaskFromNote(quadrant, taskText) {
        try {
            const fileName = this.getYearlyFileName();
            if (!fileName) return;
            ...
            const file = this.safeGetFile(fileName);
            if (!file) {
                safeLog.warn('연간 노트 파일이 존재하지 않습니다:', fileName);
                return;
            }
            ...
            if (modified) {
                await this.safeModifyFile(file, content);
                safeLog.log('연간 노트에서 할일 제거 완료:', taskText);
            }
        } catch (error) {
            safeLog.error('연간 노트에서 할일 제거 오류:', error);
        }
    }
  ...
  async createYearlyNote() {
    try {
        ...
        // 폴더 생성
        const folderPath = `${mainFolder}/${yearlyFolder}`;
        if (!await this.app.vault.adapter.exists(folderPath)) {
            await this.app.vault.createFolder(folderPath);
            safeLog.log('연간 폴더 생성:', folderPath);
        }

        const file = this.app.vault.getAbstractFileByPath(fileName);
        if (file) {
            new Notice(`📝 ${year}년 연간 노트가 이미 존재합니다`);
            // 기존 파일 열기
        ...
        new Notice(`📅 ${year}년 연간 계획 노트가 생성되었습니다`);
        
        // 생성된 파일 열기
        this.app.workspace.openLinkText(fileName, '', false);
        safeLog.log('연간 노트 생성 완료:', fileName);
    } catch (error) {
    ...
    try {
        ...
        const mainFolder = settings.mainFolder || 'Eisenhower Matrix';
        const yearlyFolder = settings.yearlyFolder || 'Yearly';
        return `${mainFolder}/${yearlyFolder}/${year}년-계획.md`;
    } catch (error) {
        safeLog.error('연간 파일명 생성 오류:', error);
        const year = new Date().getFullYear();
        return `Eisenhower Matrix/Yearly/${year}년-계획.md`;
    }
}
