...
class StudyDashboardView extends ItemView {
    ...
    async renderStats(container) {
        ...
        const know = gradeStats.S + gradeStats.A + gradeStats.B;
        const unsure = gradeStats.C + gradeStats.D;
        const dontKnow = gradeStats.E + gradeStats.F;
        
        const stats = [
            { label: '알고있음', value: know.toString(), color: '#10b981' },
            { label: '애매함', value: unsure.toString(), color: '#f59e0b' },
            { label: '모름', value: dontKnow.toString(), color: '#ef4444' }
        ];
    ...
    renderActionButtons(container) {
        ...
        resetBtn.addEventListener('click', async () => {
            const modal = new ConfirmModal(
                this.app,
                '정말 초기화하시겠습니까?',
                `${this.currentSubject} ${this.currentChapter}장의 모든 복습 기록이 초기화됩니다.`,
                async () => {
                    await this.resetChapterProgress();
                    new Notice('✅ 초기화 완료!');
                    this.renderDashboard(container);
                }
            );
    ...
    async resetChapterProgress() {
        try {
            ...
            if (!folderExists) {
                new Notice('⚠️ 폴더가 존재하지 않습니다.');
                return;
            }
            const files = this.app.vault.getMarkdownFiles().filter(file => file.path.startsWith(chapterFolder));
            ...
            for (const file of files) {
                ...
                if (frontmatterMatch) {
                    ...
                    frontmatter = frontmatter.replace(/understandings:\s*\[.*?\]/s, 'understandings: []');
                    newContent = `---\n${frontmatter}\n---${content.substring(frontmatterMatch[0].length)}`;
                }
                
                // 복습 체크박스 모두 해제
                newContent = newContent.replace(/- \[x\] 복습/g, '- [ ] 복습');
                
                await this.app.vault.modify(file, newContent);
            }
            
            console.log(`✅ ${this.currentSubject} ${this.currentChapter}장 초기화 완료 (${files.length}개 파일)`);
            new Notice(`✅ ${files.length}개 문제 초기화 완료!`);
        } catch (error) {
    ...
    async renderProblemsGrid(container) {
        ...
        statusDiv.innerHTML = `
            ...
            <div style="font-size: ${this.isMobile ? '0.95rem' : '1.05rem'}; line-height: ${this.isMobile ? '2.2' : '2'}; display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
                <div style="color: #059669;"><strong>알고있음: ${know}</strong></div>
                <div style="color: #d97706;"><strong>애매함: ${unsure}</strong></div>
                <div style="color: #dc2626;"><strong>모름: ${dontKnow}</strong></div>
            </div>
        `;
        ...
        for (let i = 1; i <= this.displayCount; i++) {
            ...
            if (problemData) {
                ...
                if (reviewCount >= 10) {
                    grade = 'S'; bgColor = '#059669'; borderColor = '#047857';
                } else if (reviewCount >= 7) {
                    grade = 'A'; bgColor = '#10b981'; borderColor = '#059669';
                } else if (reviewCount >= 5) {
                    grade = 'B'; bgColor = '#d97706'; borderColor = '#b45309';
                } else if (reviewCount >= 3) {
                    grade = 'C'; bgColor = '#f59e0b'; borderColor = '#d97706';
                } else if (reviewCount >= 1) {
                    grade = 'D'; bgColor = '#dc2626'; borderColor = '#b91c1c';
                } else {
                    grade = 'F'; bgColor = '#4b5563'; borderColor = '#6b7280';
                }
                ...
                cell.innerHTML = `
                    <div style="font-size: ${this.isMobile ? '1.3rem' : '1.1rem'}; font-weight: bold;">${i}</div>
                    <div style="font-size: ${this.isMobile ? '0.75rem' : '0.7rem'}; opacity: 0.9; margin-top: 2px;">${reviewCount}회</div>
                `;
                
                cell.title = `${i}번 | 복습 ${reviewCount}회\n클릭: 상태 변경`;
                
                // 클릭: 상태 변경 메뉴
                cell.addEventListener('click', (e) => {
                    e.preventDefault();
                    this.showStatusMenu(e, problemData.file, i);
                });
                ...
                cell.addEventListener('click', () => {
                    new StatusSelectModal(this.app, this.plugin, this.currentSubject, this.currentChapter, i, () => this.refresh()).open();
                });
            }
                ...
                try {
                    ...
                    if (frontmatterMatch) {
                        ...
                        if (numberMatch) {
                            ...
                            problemsData[number] = { reviewCount, file };
                            console.log(`  ✅ 문제 ${number}번 로드: 복습 ${reviewCount}회`);
                        }
                    }
                } catch (error) {
    ...
    showStatusMenu(event, file, problemNumber) {
        ...
        menu.style.cssText = `
            ...
            min-width: ${this.isMobile ? '280px' : '260px'};
        `;
        
        // 제목
        const title = menu.createEl('div');
        title.textContent = `${problemNumber}번 문제`;
        title.style.cssText = `
            font-size: ${this.isMobile ? '1.1rem' : '1rem'};
            font-weight: bold;
            margin-bottom: 12px;
            text-align: center;
            opacity: 0.9;
        ...
        // 복습횟수 입력
        const inputSection = menu.createDiv();
        ...
        const reviewInput = inputSection.createEl('input');
        ...
        const inputHint = inputSection.createEl('div');
        inputHint.textContent = '0 ~ 10회';
        inputHint.style.cssText = `
            font-size: 0.75rem;
            margin-top: 4px;
            opacity: 0.6;
        `;
        ...
        // 상태 버튼들
        const buttonContainer = menu.createDiv();
        ...
        const options = [
            { label: '미학습', reviewCount: 0, color: '#6b7280', bgColor: '#e5e7eb' },
            { label: '모름', reviewCount: 0, color: '#dc2626', bgColor: '#fecaca' },
            { label: '애매함', reviewCount: 0, color: '#d97706', bgColor: '#fed7aa' },
            { label: '알고있음', reviewCount: 0, color: '#059669', bgColor: '#d1fae5' }
        ];
        ...
        options.forEach(option => {
            ...
            btn.style.cssText = `
                ...
                border-radius: 8px;
                color: ${option.color};
                font-size: ${this.isMobile ? '1rem' : '0.95rem'};
                font-weight: bold;
                cursor: pointer;
                text-align: center;
                transition: all 0.2s;
            ...
            btn.addEventListener('click', async () => {
                const customReviewCount = parseInt(reviewInput.value) || 0;
                await this.updateProblemStatus(file, customReviewCount);
                if (menu.parentNode) {
                    document.body.removeChild(menu);
                }
                new Notice(`✅ ${problemNumber}번 → ${option.label} (복습 ${customReviewCount}회)`);
                this.refresh();
            });
        });
    ...
    async updateProblemStatus(file, reviewCount) {
        try {
            const content = await this.app.vault.read(file);
            let newContent = content;
            
            // frontmatter의 understandings 배열 업데이트 (100으로 고정)
            const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---/);
            if (frontmatterMatch) {
                let frontmatter = frontmatterMatch[1];
                
                // understandings 배열 생성 (reviewCount만큼 100으로 채움)
                const understandingsArray = Array(reviewCount).fill(100);
                const understandingsStr = `[${understandingsArray.join(', ')}]`;
                
                // understandings 필드 업데이트 또는 추가
                if (frontmatter.includes('understandings:')) {
                    frontmatter = frontmatter.replace(/understandings:\s*\[.*?\]/s, `understandings: ${understandingsStr}`);
                } else {
                    frontmatter += `\nunderstandings: ${understandingsStr}`;
                }
            ...
            // 복습 체크박스 업데이트
            const reviewRegex = /- \[([ x])\] 복습/g;
            const reviewMatches = [...content.matchAll(reviewRegex)];
            
            if (reviewMatches.length > 0) {
                // 기존 체크박스들을 reviewCount에 맞춰 업데이트
                let updatedCount = 0;
                newContent = newContent.replace(reviewRegex, (match) => {
                    updatedCount++;
                    return updatedCount <= reviewCount ? '- [x] 복습' : '- [ ] 복습';
                });
            }
            
            await this.app.vault.modify(file, newContent);
            console.log(`✅ ${file.path} 상태 업데이트 완료 (복습 ${reviewCount}회)`);
        } catch (error) {
            console.error('상태 업데이트 오류:', error);
            new Notice('❌ 상태 업데이트 실패');
        }
    }
...
class ConfirmModal extends Modal {
    ...
    onOpen() {
        ...
        contentEl.createEl('h2', { text: this.title, attr: { style: 'margin-bottom: 15px;' } });
        contentEl.createEl('p', { text: this.message, attr: { style: 'margin-bottom: 25px; opacity: 0.8;' } });
        
        const btnContainer = contentEl.createDiv();
        btnContainer.style.cssText = 'display: flex; gap: 10px; justify-content: center;';
        
        const cancelBtn = btnContainer.createEl('button', { text: '취소', cls: 'mod-cta' });
        cancelBtn.style.cssText = 'padding: 10px 20px; background: var(--background-modifier-border);';
        cancelBtn.addEventListener('click', () => this.close());
        
        const confirmBtn = btnContainer.createEl('button', { text: '확인', cls: 'mod-warning' });
        confirmBtn.style.cssText = 'padding: 10px 20px;';
...
class StatusSelectModal extends Modal {
    constructor(app, plugin, subject, chapter, number, onComplete) {
        super(app);
        this.plugin = plugin;
    ...
    onOpen() {
        ...
        contentEl.style.cssText = `
            padding: ${this.isMobile ? '25px' : '20px'};
            text-align: center;
        `;
        
        contentEl.createEl('h2', {
            text: `📝 ${this.number}번 문제 생성`,
            attr: { style: `font-size: ${this.isMobile ? '1.6rem' : '1.4rem'}; margin-bottom: 10px;` }
        });
        
        contentEl.createEl('p', {
            text: `${this.subject} ${this.chapter}장`,
            attr: { style: `font-size: ${this.isMobile ? '1.1rem' : '1rem'}; opacity: 0.8; margin-bottom: 30px;` }
        });
        
        contentEl.createEl('p', {
            text: '문제 상태를 선택하세요',
            attr: { style: `font-size: ${this.isMobile ? '1.05rem' : '0.95rem'}; margin-bottom: 20px; font-weight: 600;` }
        });
        ...
        buttonContainer.style.cssText = `
            ...
            gap: ${this.isMobile ? '15px' : '12px'};
            max-width: ${this.isMobile ? '100%' : '400px'};
            margin: 0 auto;
        `;
        ...
        options.forEach(option => {
            const rowContainer = buttonContainer.createDiv();
            ...
            const btn = rowContainer.createEl('button');
            btn.textContent = option.label;
            btn.style.cssText = `
                flex: 1;
                padding: ${this.isMobile ? '18px' : '15px'};
                font-size: ${this.isMobile ? '1.2rem' : '1.1rem'};
                font-weight: bold;
                color: ${option.color};
                background: ${option.bgColor};
                border: none;
            ...
            btn.addEventListener('mouseenter', () => {
                btn.style.background = option.hoverBg;
            });
            
            btn.addEventListener('mouseleave', () => {
                btn.style.background = option.bgColor;
            });
            
            const inputContainer = rowContainer.createDiv();
            ...
            input.type = 'number';
            input.value = option.reviewCount.toString();
            input.min = '0';
            input.max = '10';
            input.style.cssText = `
                width: ${this.isMobile ? '60px' : '50px'};
                padding: ${this.isMobile ? '10px 8px' : '8px 6px'};
                font-size: ${this.isMobile ? '1.1rem' : '1rem'};
                text-align: center;
                border: 2px solid var(--background-modifier-border);
                border-radius: 8px;
            ...
            inputContainer.createEl('span', {
                text: '회',
                attr: { style: `font-size: ${this.isMobile ? '1rem' : '0.9rem'}; opacity: 0.7;` }
            });
            
            btn.addEventListener('click', async () => {
                const customReviewCount = parseInt(input.value) || option.reviewCount;
                await this.createProblem(customReviewCount);
                this.close();
            });
        });
    }
    
    async createProblem(reviewCount) {
        try {
            const folderPath = normalizePath(`${this.plugin.settings.problemsFolder}/${this.subject}/${this.chapter}장`);
            ...
            const fileName = `${this.number}.md`;
            const filePath = normalizePath(`${folderPath}/${fileName}`);
            
            // understandings 배열 생성
            const understandingsArray = Array(reviewCount).fill(100);
            
            // 복습 체크박스 생성
            let reviewChecks = '';
            for (let i = 0; i < 10; i++) {
                const checked = i < reviewCount ? 'x' : ' ';
                reviewChecks += `- [${checked}] 복습\n`;
            }
...
understandings: [${understandingsArray.join(', ')}]
...
`;
            ...
            await this.app.vault.create(filePath, content);
            new Notice(`✅ ${this.number}번 문제 생성 완료! (복습 ${reviewCount}회)`);
