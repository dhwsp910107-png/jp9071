...
class PageProgressDashboardModal extends Modal {
    ...
    onOpen() {
        ...
        // 대시보드 내 아이콘/버튼 UI
        const buttonDiv = contentEl.createDiv({ cls: 'dashboard-action-buttons' });
        ...
        // 설정 버튼
        const settingsBtn = buttonDiv.createEl('button', { text: '⚙️ 설정' });
        settingsBtn.onclick = () => {
            this.close();
            this.app.setting.open();
            this.app.setting.openTabById('page-progress-manager');
        };

        // 블록 생성 버튼
        const blockBtn = buttonDiv.createEl('button', { text: '📚 블록 시스템 생성' });
        blockBtn.onclick = () => {
            this.close();
            new PageBlockModal(this.app, this.settings, (data) => {
                // PageBlock 시스템 생성 로직
        ...
        // 새로고침 버튼
        const refreshBtn = buttonDiv.createEl('button', { text: '🔄 새로고침' });
        refreshBtn.onclick = () => {
            this.close();
            new PageProgressDashboardModal(this.app, this.settings).open();
        };
...
class PageProgressSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        // 모바일 최적화 대시보드
        const dashboardDiv = containerEl.createDiv({ cls: 'mobile-dashboard' });
        dashboardDiv.style.display = 'flex';
        dashboardDiv.style.flexDirection = 'column';
        dashboardDiv.style.gap = '16px';
        dashboardDiv.style.marginTop = '24px';

        dashboardDiv.createEl('h3', { text: '📊 전체 진도 현황' });

        // 최근 수정 파일 리스트
        const recentDiv = dashboardDiv.createDiv();
        recentDiv.createEl('h4', { text: '🕒 최근 수정한 파일' });

        // 파일 리스트 가져오기
        const files = this.app.vault.getMarkdownFiles()
            .filter(f => f && f.parent && f.parent.path.startsWith(this.plugin.settings.progressFolder));
        const sorted = files.sort((a, b) => b.stat.mtime - a.stat.mtime).slice(0, 5);

        const ul = recentDiv.createEl('ul');
        sorted.forEach(file => {
            const li = ul.createEl('li');
            li.createEl('a', { text: file.name, href: '#' }).onclick = () => {
                this.app.workspace.getLeaf().openFile(file);
            };
            li.createSpan({ text: ` (${new Date(file.stat.mtime).toLocaleDateString()})` });
        });

        // 전체 진도 계산
        const completed = files.filter(f => f.basename.includes('완료')).length;
        const total = files.length;
        const percent = total ? Math.round((completed / total) * 100) : 0;

        dashboardDiv.createEl('div', {
            text: `진도 완료: ${completed} / ${total} (${percent}%)`
        });

        const bar = dashboardDiv.createDiv();
...
// 스터디 대시보드 플러그인 템플릿 구조 (문제/블록/노트 생성용)
const STUDY_DASHBOARD_TEMPLATE = ({number, title, subject, difficulty, today}) => `---
number: ${number}
title: "${title}"
subject: "${subject}"
status: "unknown"
